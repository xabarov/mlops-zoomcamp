...
def search_best_params(search_space, max_evals: int = 3):
    ...
    best_result = {
        k: v.tolist() if isinstance(v, np.ndarray) else v.item() if isinstance(v, np.float64) else v
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--search_space_params_path',
        type=str,
        default='search_space_params.yaml',
        help='Path to search space params',
    )
    ...
    parser.add_argument(
        '--save_path',
        type=str,
        default='nyc/best_train_params.yaml',
    ...
    learning_rate_distr = distributions[search_params['learning_rate']['distribution']]
    reg_alpha_distr = distributions[search_params['reg_alpha']['distribution']]
    reg_lambda_distr = distributions[search_params['reg_lambda']['distribution']]
    ...
    SEARCH_SPACE = {
        'max_depth': scope.int(
            max_depth_distr(
                'max_depth',
                search_params['max_depth']['low'],
                search_params['max_depth']['high'],
                search_params['max_depth']['q'],
            )
        ),
        'learning_rate': learning_rate_distr(
            'learning_rate',
            search_params['learning_rate']['low'],
            search_params['learning_rate']['high'],
        ),
        'reg_alpha': reg_alpha_distr(
            'reg_alpha', search_params['reg_alpha']['low'], search_params['reg_alpha']['high']
        ),
        'reg_lambda': reg_lambda_distr(
            'reg_lambda', search_params['reg_lambda']['low'], search_params['reg_lambda']['high']
        ),
        'min_child_weight': min_child_weight_distr(
            'min_child_weight',
            search_params['min_child_weight']['low'],
            search_params['min_child_weight']['high'],
        ),
    ...
    best_params = search_best_params(
        search_space=SEARCH_SPACE, max_evals=search_params['max_evals']
    )
