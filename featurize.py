...
def feature_engineering(
    ...
    df = df[(df.duration >= duration_minutes_min) & (df.duration <= duration_minutes_max)]
    ...
    drop_columns = [
        ...
        if feature not in features['categorical'] + ['duration'] + features['numerical']
    ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Feature engineering for NYC taxi dataset')
    ...
    parser.add_argument(
        '--features_path', type=str, default='features.yaml', help='Path to the features.yaml file'
    )

    parser.add_argument(
        '--save_path', type=str, default='nyc/processed/yellow_tripdata_processed.parquet'
    )
    ...
    with open(args.features_path, 'r', encoding='utf-8') as f:
        features_dict = yaml.safe_load(f)['features']
